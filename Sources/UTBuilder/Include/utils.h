...
public:
   ...
   static const char* getDeclSourceFile(const clang::Decl           *decl,
                                        const clang::SourceManager  &srcMgr);
   // append the row and column to the file name name string 
   static const char* getDeclSourceFileLine(const clang::Decl *decl,
                                            const clang::SourceManager &srcMgr);   
   static const char* getStmtSourceFile(const clang::Stmt           *stmt,
                                        const clang::SourceManager  &srcMgr);
   // append the row and column to the file name name string 
   static const char* getStmtSourceFileLine(const clang::Stmt          *stmt,
                                            const clang::SourceManager &srcMgr);   
};
